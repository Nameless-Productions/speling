...
export const load: PageServerLoad = async ({url}) => {
    ...
    const posts: {author: string, content: string, likes: number, date: Date, comments: {author: string, content: string}[]}[] = []
    ...
    for (const post of postsDB) {
        ...
        posts.push({author: user.username, content: post.content, likes: likes.length, date: post.createdAt, comments})
    }

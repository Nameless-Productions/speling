...
<div class="flex h-screen items-center justify-center">
	<div class="rounded-xl border-2 p-5">
		...
		<button onclick={() => {
			if (agreeToThings) {
				location.href = data.discord
			}
		}} disabled={!agreeToThings} class="rounded-xl bg-blue-600 p-1 cursor-pointer disabled:cursor-not-allowed duration-200 hover:bg-blue-800"
			>Sign in with Discord</button
		>
		<br>
		<div class="flex">
			<input type="checkbox" class="m-1" bind:checked={agreeToThings}>
			<p>I agree to the <br><button
				type="button"
